...
const keyOf = (image: ResponsiveImage | null): string | null =>
  image?.webp[0]?.src ?? null;
...
const reducer = (state: State, action: Action): State => {
  switch (action.type) {
    case "target": {
      ...
      return {
        ...state,
        incoming: { key, image: action.image },
 ...
 */
export const useWallpaperCrossfade = (
  ...
  const handleLoad = useCallback(
    (key: string) => dispatch({ type: "load", key }),
    [],
  );
  const handleError = useCallback(
    (key: string) => dispatch({ type: "error", key }),
    [],
  );
  const commit = useCallback(
    (key: string) => dispatch({ type: "commit", key }),
    [],
  );
