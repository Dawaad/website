...
import {
  useWallpaperCrossfade,
  type WallpaperLayer,
} from "@/src/features/portfolio/hooks/use-wallpaper-crossfade";
...
export const Crossfade: FC<CrossfadeProps> = ({
  ...
  const layer = (entry: WallpaperLayer, opacity: string, handlers = false) => (
    <picture key={entry.key} className="absolute inset-0 block">
      {entry.image.avif && (
        <source
          srcSet={buildSrcSet(entry.image.avif)}
          type="image/avif"
          sizes={sizes}
        />
      )}
      <source
        srcSet={buildSrcSet(entry.image.webp)}
        type="image/webp"
        sizes={sizes}
      />
      <img
  ...
  return (
    <>
      ...
      {incoming &&
        layer(incoming, incomingLoaded ? "opacity-100" : "opacity-0", true)}
    </>
  );
};
