...
const TargetCursor: React.FC<TargetCursorProps> = ({
    ...
    useEffect(() => {
        ...
        // Initially hide cursor if no container specified
        if (!containerRef) {
        ...
        const containerEnterHandler = () => {
            ...
            setLiquidCursorVisible(false); // Hide liquid cursor
        ...
        const containerLeaveHandler = () => {
            ...
            setLiquidCursorVisible(true); // Show liquid cursor
            gsap.to(cursorRef.current, { autoAlpha: 0, duration: 0.2 });
        };

        if (containerRef?.current) {
            containerRef.current.addEventListener("mouseenter", containerEnterHandler);
            containerRef.current.addEventListener("mouseleave", containerLeaveHandler);
        }
        ...
        return () => {
            ...
            if (containerRef?.current) {
                containerRef.current.removeEventListener("mouseenter", containerEnterHandler);
                containerRef.current.removeEventListener("mouseleave", containerLeaveHandler);
            }
            if (activeTarget) {
                cleanupTarget(activeTarget);
            }
