...
const ELSEWHERE: ElsewhereLink[] = [
  { when: 'github', handle: 'github.com/kade', note: 'open source · 24 repos · 1.2k followers' },
  { when: 'read.cv', handle: 'read.cv/kade', note: 'résumé · long-form work history' },
  { when: 'are.na', handle: 'are.na/kade-okafor', note: 'references · 38 channels · slowly updated' },
  { when: 'mastodon', handle: '@kade@hci.social', note: 'quiet account · mostly reading' },
  { when: 'last.fm', handle: 'last.fm/user/kade-o', note: 'scrobbling since 2009 — yes really' },
];
    ...
    <>
      ...
      <Panel label="~/elsewhere" meta="4 channels">
        <div className="mb-2 flex items-center gap-2.5 overflow-hidden whitespace-nowrap border-b border-fg-4 pb-2.5 text-[12px] text-fg-2">
          ...
          <span className="min-w-0 truncate text-fg-0">curl https://kade.signal.so/links</span>
        </div>
        <div className="flex flex-col">
          {ELSEWHERE.map((l) => (
            ...
            >
              <span className="text-[11px] tracking-[0.06em] text-fg-3">{l.when}</span>
              <span className="text-fg-1">
                {l.handle}
                <small className="mt-[3px] block text-[11px] text-fg-3">{l.note}</small>
              </span>
            </div>
          ))}
        </div>
